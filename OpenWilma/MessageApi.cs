...
public class MessageApi : IMessageApi
{
    ...
    public MessageApi(IWilmaSession session, Role role)
        ...
        params (RecipientType type, string id)[] recipients)
    {
        var parameters = new List<KeyValuePair<string, string>>
        {
            ...
            new("ShowRecipients", showRecipients.ToString()),
            new("CollatedReplies", collatedReplies.ToString())
        };

        foreach (var (recipientType, recipientId) in recipients)
        {
            if (recipientType == RecipientType.Guardian)
                throw new NotImplementedException(nameof(RecipientType.Guardian));

            parameters.Add(new("r_" + recipientType.ToString(), recipientId.ToString()));
        }
    ...
    public async Task<bool> ArchiveAsync(params int[] messageIds)
    {
        ...
        foreach (int mid in messageIds)
        {
            parameters.Add(new("mid", mid.ToString()));
        }
